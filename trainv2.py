...
def weak_loss(model, batch, normalization="softmax", alpha=30):
    ...
    #size=batch["source_image"].size()
    #print(size)
    print(batch["source_image"][1])
    print(batch["source_image"][np.roll(np.arange(b), -1), :][0])
    ...
    # negative
    #这句话的意义就是说本来source图片序号是[0,1,2,3,...] target图片序号是[0,1,2,3,...] 0,和0...一一对应的现在对source
    #图片序号进行调整source图片序号变为[1,2,3,...,0] target图片序号仍然为[0,1,2,3...], 这样这么一对一对的图片就是负样本了
